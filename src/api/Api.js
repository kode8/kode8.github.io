...
/* @param id : string */
const getEntries = query => fetchEntries(query)
  .then(entries => entries);

/* Retrieves A Single entry */
/* @param entry_id : string */
const getEntry = entry_id => fetchEntry(entry_id)
  .then(entry => entry);

/* Retrieves ContentTypes */
/* @param id : string */
const getContentType = id => fetchContentType(id)
  .then(type => type);

/* Retrieves All ContentTypes */
const getContentTypes = () => fetchContentTypes()
  .then(types => types);

/* Contentful Api request to get a Content Entries */
/* @param id : string | navigation */
const fetchEntry = entry_id => Client.getEntry(entry_id)
  .then(entry => entry)
  .catch((error) => {
...
/* Contentful Api request to get a Content Entries */
/* @param id : string | navigation */
const fetchEntries = query => Client.getEntries(query)
  .then(entries => entries)
  .catch((error) => {
...
const fetchContentType = id => Client.getContentType(id)
  .then(type => type)
  .catch((error) => {
